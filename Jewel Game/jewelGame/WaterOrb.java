...
public class WaterOrb extends GameWorldObject implements IDrawable, ISelectable, IMovable, IDeletable
{
	private File waterOrbFile;
    ...
    public WaterOrb(Point2D objectLocation)
    {
    	waterOrbFile = new File("." + File.separator + "Images" + File.separator + "Water Orb.png");
    ...
    //Drawing from the center of the orb. The picture is 120x120.
  	public void draw(Graphics g)
  	{
		...
		try 
		{
			BufferedImage bufferedImage = ImageIO.read(waterOrbFile);
	...
	{
		return "Water Orb: loc="+getLocation().getX()+","+getLocation().getY();
	}
