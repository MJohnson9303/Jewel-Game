...
public class FireOrb extends GameWorldObject implements IDrawable, ISelectable, IMovable, IDeletable
{
	...
	private int deletionImageSize = 120;
    ...
    public FireOrb(Point2D objectLocation)
    {
    	fireOrbFile = new File("." + File.separator + "Images" + File.separator + "Fire Orb.png");
    ...
    //Drawing from the center of the orb. The picture is 120x120.
  	public void draw(Graphics g)
  	{
		try 
		{
			BufferedImage bufferedImage = ImageIO.read(fireOrbFile);
	...
	{
		return "Fire Orb: loc="+getLocation().getX()+","+getLocation().getY();
	}
