...
public class DarknessOrb extends GameWorldObject implements IDrawable, ISelectable, IMovable, IDeletable
{
	private File darknessOrbFile;
    ...
    public DarknessOrb(Point2D objectLocation)
    {
    	darknessOrbFile = new File("." + File.separator + "Images" + File.separator + "Darkness Orb.png");
    ...
    //Drawing from the center of the orb. The picture is 120x120.
  	public void draw(Graphics g)
  	{
		...
		try 
		{
			BufferedImage bufferedImage = ImageIO.read(darknessOrbFile);
	...
	{
		return "Darkness Orb: loc="+getLocation().getX()+","+getLocation().getY();
	}
