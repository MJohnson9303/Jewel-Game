...
public class LeafOrb extends GameWorldObject implements IDrawable, ISelectable, IMovable, IDeletable
{
	private File leafOrbFile;
    ...
    public LeafOrb(Point2D objectLocation)
    {
    	leafOrbFile = new File("." + File.separator + "Images" + File.separator + "Leaf Orb.png");
    ...
    //Drawing from the center of the orb. The picture is 120x120.
  	...
  	{	
		try 
		{
			BufferedImage bufferedImage = ImageIO.read(leafOrbFile);
	...
	{
		return "Leaf Orb: loc="+getLocation().getX()+","+getLocation().getY();
	}
